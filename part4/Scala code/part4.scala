...
object Main{
	class TokenizerMapper extends Mapper[Object, Text, Text, IntWritable]{
		...
		override def map(key: Object, value: Text, context: Mapper[Object, Text, Text, IntWritable]#Context): Unit = {
			...
			val arr = line.split(",")
			val hour = ((arr(0).split(" "))(1).split(":"))(0).toInt
			...
			if((hour>=20 || hour<=1) && pickLat.toFloat!=0.0 && pickLon.toFloat!=0.0 && dropLat.toFloat!=0.0 && dropLon.toFloat!=0.0)
			{
				val k: String  = month+","+pickLat+","+pickLon
				val c = arr(7).toInt
				val foo = mm.get(k)
				foo match{
					case Some(x) =>	
				...
				val nk : String = month+","+dropLat+","+dropLon
				val nfoo = mm.get(nk)
				nfoo match{
					case Some(x) =>	
						mm.put (k,x+c)
